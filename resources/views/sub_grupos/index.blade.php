<x-app>
    ...
    @slot('slot')
        <div class="row g-5 g-xl-8">
                ...
                <div class="card">
                    <div class="card h-100 ">
                        ...
                        <div class="card-body d-flex flex-column px-9 pt-6 pb-8">
                            <div class="fs-2tx fw-bold mb-14">
                                {{ $sub_grupos_all }}
                            </div>
            ...
            <div class="col-xl-8 col-md-6 col-12">
                <div class="card">
                    <div class="card h-100 ">
                        ...
                        <form action="{{ route('sub_grupos.store') }}" method="POST">
                            @csrf
                            <div class="card-body">
                                <div class="row">
                                    <div class="col-xl-12 col-md-12 col-12">
                                        <div class="row">
                                            ...
                                            <div class="col-6 form-group">
                                                <label for="nome" class="form-label fs-6 fw-bold mb-3 required">Nome</label>
                                ...
                                <div class="card-body">
                                    <div class="col-xl-12 col-md-12 col-12">
                                        <div class="form-group mb-3">
                                            <label for="nome_editar" class="form-label fs-6 fw-bold mb-3 required">Nome</label>
                                            <input type="text" class="form-control" value="" id="nome_editar" name="nome" placeholder="Nome" required>
                                        </div>
                                        <div class="form-group">
                                            <label for="grupo" class="form-label fs-6 fw-bold mb-3 required">Grupo</label>
                                            <select name="grupo" id="grupo_editar" class="form-select form-select-solid fw-bold" data-kt-select2="true"
                                                    data-placeholder="Selecione" data-allow-clear="true"
                                                    data-kt-user-table-filter="role" data-hide-search="true" required>
                                                <option value="">Selecione</option>
                                                @foreach($grupos as $grupo)
                        ...
                        <div class="card-toolbar">
                            <div class="d-flex justify-content-end" data-kt-user-table-toolbar="base">
                                ...
                                <div class="menu menu-sub menu-sub-dropdown w-300px w-md-325px" data-kt-menu="true">
                                    ...
                                    <form action="{{ route('sub_grupos.index') }}" method="GET" class="px-7 py-5" data-kt-user-table-filter="form">
                                        <div class="mb-10">
                                ...
                                <table class="table align-middle table-row-dashed fs-6 gy-5">
                                    ...
                                    @foreach ($sub_grupos as $sub_grupo)
                                        <tr>
                                            ...
                                            <td>{{ $sub_grupo->grupo }}</td>
                                            <td>{{ date('d/m/Y H:i', strtotime($sub_grupo->created_at)) }}</td>
                                            <td class="text-center">
                                                ...
                                                <div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-semibold fs-7 w-125px py-4"
                                                    ...
                                                    <div class="menu-item px-3">
                                                        <a href="#"
                                                           class="menu-link px-3" id="editar_modal" onclick="editarModal({{ $sub_grupo->id }}, `{{ $sub_grupo->grupo }}`, `{{ $sub_grupo->nome }}`)">
                                                            Editar
                                                        </a>
                                                    </div>
                                                    <div class="menu-item px-3">
                                                        <form id="form-delete" action="{{ route('sub_grupos.destroy', $sub_grupo->id) }}" method="post" class="menu-link px-3">
                                                            @csrf
                                                            @method('DELETE')
            ...
            <div class="pr-4 pl-4">
                @if ($sub_grupos->hasPages())
                    <div class="d-flex justify-content-between">
                        <div id="mostrarPaginas">
                            <p class="text-sm text-gray leading-5">
                                Mostrando
                                <span class="font-medium">{{ $sub_grupos->firstItem() }}</span>
                                até
                                <span class="font-medium">{{ $sub_grupos->lastItem() }}</span>
                                de
                                <span class="font-medium">{{ $sub_grupos->total() }}</span>
                                resultados
                            </p>
                        </div>
                        {{ $sub_grupos->links() }}
    ...
    @slot('scripts')
        <script>
            function editarModal(id, grupo, nome){
                let action = "{{ route('sub_grupos.update', 'id_grupo') }}".replace('id_grupo', id)
                $('#modal_editar').modal('show')
                $('#form_editar').attr('action', action)
                $('#nome_editar').val(nome)
                $('#grupo_editar').val(grupo)
                $('#grupo_editar').trigger('change')
            }
        </script>
    @endslot
</x-app>
