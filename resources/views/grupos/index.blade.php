                        ...
                        <div class="card-toolbar">
                            <div class="d-flex justify-content-end" data-kt-user-table-toolbar="base">
                                ...
                                <div class="menu menu-sub menu-sub-dropdown w-300px w-md-325px" data-kt-menu="true">
                                    ...
                                    <form action="{{ route('grupos.index') }}" method="GET" class="px-7 py-5" data-kt-user-table-filter="form">
                                        <div class="mb-10">
                                ...
                                <table class="table align-middle table-row-dashed fs-6 gy-5">
                                    ...
                                    @foreach ($grupos as $grupo)
                                        <tr>
                                            ...
                                            <td>#{{ $grupo->id }}</td>
                                            <td>{{ $grupo->nome }}</td>
                                            <td>{{ date('d/m/Y H:i', strtotime($grupo->created_at)) }}</td>
                                            <td class="text-center">
                                                ...
                                                <div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-semibold fs-7 w-125px py-4"
                                                    ...
                                                    <div class="menu-item px-3">
                                                        <a href="#"
                                                           class="menu-link px-3" id="editar_modal" onclick="editarModal({{ $grupo->id }}, `{{ $grupo->nome }}`)">
                                                            Editar
                                                        </a>
                                                    </div>
                                                    <div class="menu-item px-3">
                                                        <form id="form-delete" action="{{ route('grupos.destroy', $grupo->id) }}" method="post" class="menu-link px-3">
                                                            @csrf
                                                            @method('DELETE')
                    ...
                    <div class="d-flex justify-content-between">
                        <div id="mostrarPaginas">
                            <p class="text-sm text-gray leading-5">
                                Mostrando
                                <span class="font-medium">{{ $grupos->firstItem() }}</span>
                                até
                                <span class="font-medium">{{ $grupos->lastItem() }}</span>
                                de
                                <span class="font-medium">{{ $grupos->total() }}</span>
                                resultados
                            </p>
                        </div>
                        {{ $grupos->links() }}
    ...
    @slot('scripts')
        <script>
            function editarModal(id, nome){
                let action = "{{ route('grupos.update', 'id_grupo') }}".replace('id_grupo', id)
